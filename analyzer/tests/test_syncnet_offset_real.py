"""Validates the pretrained SyncNet estimator against real content with a
known, injected ground-truth offset, using the same methodology as
`test_mouth_offset_real.py`, which used this to falsify the classical
mouth-motion heuristic. This one is the positive result: unlike that
heuristic and the Tier-1 coarse detector, SyncNet actually recovers the
injected offset.

Skipped if either the real-content fixture or the vendored syncnet_python
and weights aren't present (`analyzer/scripts/fetch_real_content.sh` and
...
deliberately covers only 2 cases, not a full sweep.
"""
...
def test_syncnet_recovers_known_real_offset(tmp_path, injected_ms):
    ...
    # offset magnitude; this is a different scale than the coarse
    # detector's [0, 1]-ish correlation.
