...
"expected" choice for this kind of work: empirically (see
`docs/RESEARCH.md`), mediapipe's modern Tasks API initializes a Metal-backed
image pipeline on macOS *even when a CPU delegate is explicitly requested*,
and that crashes with `Check failed: service_ Service is unavailable` in
this project's execution environment. Rather than fight a GPU-service
dependency for what is fundamentally a CPU-sized model, this module uses
`cv2.FaceDetectorYN` -- a small (~230KB) ONNX model that runs through
OpenCV's own DNN backend with no GPU dependency at all. Same job (real face
detection on real video), no platform-specific crash.
